...
- 1_geometry_and_mask.py  (Step 1)  
  - 作用：对齐海网格到陆网格（使两者共用同一 global domain Ω），构建优先掩模 m(x,y)（海=1、仅陆=0、无数据=NaN），并生成初始复合场 g0(x,y)（仅用于可视化/检查）。  
  - 输出：
...
- 2_laplacian_computation.py  (Step 2)  
  ...
  - 输出：
    ...
    - `Sea_Laplacian_Aligned.asc` — 将海 Laplacian 对齐到陆格网的结果（用于合并）。

- 3_merge_and_interpolate_dual.py  (Step 3 & 4)  
  - 作用：使用优先掩模合并两个 Laplacian（Equation (15)），生成合并后源场 L0；识别因 stencil 边界产生的 gap（Ω_gap）；对 gap 区域做插值填充（支持 `INTERP_MODE='global'` 或 `local`，local 支持 `PADDING` 参数）。
  - 中间/输出文件：
...
- 5_frequency_inversion.py  (Step 5)  
  - 作用：将填充后的 Laplacian 在频率域反演得到最终 Bouguer 异常场（使用离散 Laplacian 的传递函数 H(kx,ky)）；支持直接除以 H（默认）或 Tikhonov 正则化（`--tikhonov` 与 `--alpha`）。
  - 默认输入/输出（可通过命令行参数覆盖）：
    - 输入：`L_FilledLaplacian.asc` (`--input`)
...
- 6_validate_bouguer_merge.py  
  - 作用：在陆地仅区间比较原始陆地网格与合并后最终结果，计算差值统计并可选择性地对 final 网格做系统性偏移校正（`--correct`）。
  - 使用示例：  
...
9. `BouguerFinal.asc` / `BouguerFinal_Tikhonov.asc` — 由 `5_frequency_inversion.py` 生成  
   - 内容：由插值后的 Laplacian 在频率域反演得到的最终 Bouguer 异常网格（无正则 / Tikhonov 版本）。

10. `BouguerFinal_Corrected.asc`（可选） — 由 `6_validate_bouguer_merge.py --correct` 生成  
...
- 注意：脚本中有默认的硬编码目录 `C:/.../BouguerAnomaly/StitchGrids`，在执行前建议修改脚本顶部的 `data_dir` / `output_dir` 或将当前工作目录切换到包含 .asc 文件的目录并相应调整脚本常量。
...
- 文件格式的一致性：各脚本对 nodata 的约定不同（有的用 -9999，有的根据 header nodata_value），运行前请确保输入 .asc 的 nodata 与脚本一致，或调整脚本中的 nodata 替换逻辑。
- 已发现代码问题（建议修复）：
  - `6_validate_bouguer_merge.py` 中 `parse_args()` 添加 `--threshold` 时参数一行缺少逗号（会导致语法错误）；该文件内后续打印字符串使用 f-string 拼接处也有格式问题（需要修正），建议在运行前修复这些小错误。
- 插值方法对结果敏感：`griddata(method='cubic')` 在大网格或点数很少时可能失败或运行非常慢，local 模式更稳健。若发现大量残留 NaN，脚本会尝试最近邻填充作为最后手段，但请人工检查边界平滑性。
- Document2.md 为算法数学推导与实现说明，请优先参考它以确认参数设定与理论一致性。
...
7. 频域反演回到 Bouguer 场（可选 Tikhonov）：  
...
8. 验证并（可选）根据陆地仅区执行偏移校正：`python 6_validate_bouguer_merge.py --land BouguerLandGrid.asc --sea BouguerSeaGrid.asc --final BouguerFinal.asc --correct --output BouguerFinal_Corrected.asc --threshold 1.0`
